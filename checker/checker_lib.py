...
class CheckMachine:
    ...
    def create_appointment(self, appointment: Appointment):
        return self.s.post(f'{self.url}/appointment', dict = appointment, timeout = self.timeout)
    ...
    def generate_patient():
        ...
        last_name=mimesis.Person().last_name(),
        email=mimesis.Person().email(),
        birth_year=mimesis.Person().birthdate(min_year=2222).year,
    ...
    def generate_service(vip = False, description = 'Nothing interesting'):
        ...
        return asdict(Service(name=choice(services), description=description, vip=vip))